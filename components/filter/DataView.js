...
var DataView = React.createClass({
  ...
  async showPickerTime(stateKey, options) {
    try {
      ...
      if (action === TimePickerAndroid.timeSetAction) {
        ...
        newState[stateKey + 'Text'] = afterTime;
      }
  ...
  async showPicker(stateKey, options) {
     ...
     try {
      ...
      if (action === DatePickerAndroid.dismissedAction) {
        newState[stateKey + 'Text'] = afterData;
      } else {
  ...
  render: function () {
    afterTime = this.state.isoFormatText;
    afterData = this.state.allText;
    return (
      <View style ={{height:style.smallView,marginTop:30}}>
          ...
          <View style={{width:16}}></View>
           <View style ={{flex: 1,borderColor:style.backgroundColor, borderWidth: style.borderWidth,borderBottomColor:style.borderColor ,flexDirection: 'column',justifyContent: 'space-between'}}>
            <View style={{marginTop:16}}>
              <Text style = {{color:style.colorTextStandart,fontSize: style.activeTextFont}}>По</Text>
            </View>
            <View style ={{flexDirection: 'row',justifyContent: 'space-between',marginBottom:16,}}>
                ...
                <View style = {{flexDirection: 'row'}}>
                   <View>
                      ...
                      <Text style = {{fontSize:style.inactiveTextFont, marginTop:5, marginRight:10}}>{afterTime}</Text>
                      <View>
